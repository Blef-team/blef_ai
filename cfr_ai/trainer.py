...
class Trainer():
    ...
    def get_node_value(self, hands: List[List[str]], history: List[int], reach_probability: float, active_player: int, mc_player: int, prune_feast: bool):
        ...
        if active_player == mc_player:
            ...
            for i, action in enumerate(possible_actions):
                if info_set.regrets[i] >= -300 or prune_feast:
                    counterfactual_values[i] = -self.get_node_value(hands, history + [action], reach_probability * strategy[i], opponent, mc_player, prune_feast)
            ...
            node_value = -self.get_node_value(hands, history + [action], reach_probability, opponent, mc_player, prune_feast)
