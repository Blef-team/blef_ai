    ...
    def get_node_value(self, hands: List[List[str]], history: List[int], reach_probability: float, active_player: int, mc_player: int, warm_up: bool, prune_feast: bool):
        ...
        if active_player == mc_player:
            ...
            for i in range(info_set.num_actions):
                if info_set.regrets[i] >= -1000 or prune_feast:
                    counterfactual_values[i] = -self.get_node_value(hands, history + [info_set.possible_actions[i]], reach_probability * strategy[i], opponent, mc_player, warm_up, prune_feast)
                else:
                    counterfactual_values[i] = 0
            node_value = counterfactual_values.dot(strategy)
            for i in range(info_set.num_actions):
                if info_set.regrets[i] >= -1000:
                    info_set.regrets[i] += counterfactual_values[i] - node_value
                elif prune_feast:
                    info_set.regrets[i] += 10 * (counterfactual_values[i] - node_value)
        else:
