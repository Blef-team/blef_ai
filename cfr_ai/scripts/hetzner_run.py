...
PROGRESS_PATH = os.path.join(OUTPUTS_DIR, "_progress.json")
...
def _setup_dir(setup: Tuple[int, int]) -> str:
    return os.path.join(OUTPUTS_DIR, f"{setup[0]}_{setup[1]}")


def _train_output_exists(setup: Tuple[int, int]) -> bool:
    d = _setup_dir(setup)
    return os.path.exists(os.path.join(d, "strategy.npz")) and \
           os.path.exists(os.path.join(d, "diagnostic.npz"))


def _lbr_done_at_depth(setup: Tuple[int, int], depth_label: str) -> bool:
    ...
    path = os.path.join(OUTPUTS_DIR, "summary_of_all_runs.csv")
...
def _build_train_job(setup: Tuple[int, int], iters: int, log_points: int) -> Job:
    ...
    cmd = [
        ...
        "--penalty", cfg["penalty"],
        "--min-bet", cfg["min_bet"],
        "--pruning-range", cfg["pruning"], cfg["min_regret"],
    ...
    log_path = os.path.join(_setup_dir(setup), "train.log")
    return Job(setup=setup, phase="train", cmd=cmd, log_path=log_path)


def _build_lbr_job(setup: Tuple[int, int], depth: int) -> Job:
    cmd = [
    ...
    log_path = os.path.join(_setup_dir(setup), f"lbr{depth}.log")
...
def _build_phase_queue(phase: str, iters: int, log_points: int) -> List[Job]:
    ...
    for setup in _all_setups():
        if phase == "train":
            if _train_output_exists(setup):
                continue  # resume: skip if strategy + diagnostic already present
            jobs.append(_build_train_job(setup, iters, log_points))
        elif phase == "lbr1":
            if _lbr_done_at_depth(setup, "1"):
                continue
            jobs.append(_build_lbr_job(setup, depth=1))
        elif phase == "lbr2":
            if _lbr_done_at_depth(setup, "2"):
                continue
            jobs.append(_build_lbr_job(setup, depth=2))
        else:
